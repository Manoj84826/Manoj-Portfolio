<img width="282" height="282" alt="image" src="https://github.com/user-attachments/assets/90cf5374-3294-4806-a2ae-0bb3eeb3e612" />


# Personal information: 
Address: Glanzstr 
12437, Berlin 
Mobile Number: +49 17624547355 / +91 6302640772
Email ID: valammanoj84@gmail.com 
Date of Birth: 23.12.1999
Gender: Male
Languages: A2 German, C2 English

### PROFESSIONAL SUMMARY: 
Detail-oriented and motivated postgraduate student currently pursuing an MSc
in Data Science at Arden University, Berlin, with a strong academic foundation in
Electronics and Communication Engineering. Aspiring Data Analyst / Data
Science professional with hands-on knowledge of data analysis, statistical
concepts, and programming. Skilled in transforming data into actionable insights
using analytical tools, with strong problem-solving ability, teamwork, and
adaptability.

# Data Scientist

### Education: 
• MSc in Data Science, Arden University, Pursuing 
• B.Tech - Electronics & Communication Engineering 65%, year 2017 to 2023

### Project working on:
• Currently conducting thesis research on comparative Machine Learning
techniques for medical image analysis, with a focus on lung disease detection.
• Developed and implemented Machine Learning models, including a flight
price prediction system using real-world datasets.
• Completed data visualisation projects, transforming complex datasets into
meaningful and actionable insights.
• Utilised Python and SQL for data cleaning, data analysis, modelling, and
interpretation.
• Applied statistical and analytical techniques to work with large datasets and
present findings effectively through structured academic writing

### Project completed: 
• Artificial Intelligence and Neural Networks 
• Big Data and Cloud Computing
• Data Visualisation
• Machine Learning
• Mathematics for Data Science
• Programming for Data Analytics
• Academic Writing in English 2 (UK)
• Academic Writing in English 1 (Berlin)

### DATA SCIENCE & DATA ANALYST SKILLS:
• Data Cleaning & Data Preprocessing
• Exploratory Data Analysis (EDA)
• Data Visualization & Reporting
• Statistical Analysis (Descriptive Statistics, Probability Basics)
• Feature Engineering (Basics)
• Data Interpretation & Business Insights
• SQL Queries for Data Extraction
• Working with Structured Datasets

### Certification: 
• Python for Beginners– Completion Certificate
• Java Basics – Workshop Certificate
• Fundamentals of React.js – Workshop Certificate
• Programming with C & C++ – Training Certificate

### SOFT SKILLS & PERSONAL TRAITS: 
• Strong analytical and logical thinking
• Excellent communication and teamwork skills
• Hardworking and disciplined
• Problem-solving mindset
• Adaptable to new tools and technologies
• Positive attitude and decision-making ability
• Embedded Systems – Participation Certificate
• Embedded Systems Workshop – Tesla EV

### TECHNICAL SKILLS: 
Python | SQL | Machine Learning | Data Analysis | Data Visualisation |
Statistical Analysis | Predictive Modelling
